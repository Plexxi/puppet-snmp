...
class snmp::params {
  ...
  $snmp_auth_trap_enable = getvar('::snmp_auth_trap_enable')
  if $snmp_auth_trap_enable {
    $auth_trap_enable =  $::snmp_auth_trap_enable
  } else {
    $auth_trap_enable =  false
  }
  ...
  $snmp_trap_sink = getvar('::snmp_trap_sink')
  if $snmp_trap_sink {
  ...
  $snmp_trap2_sink = getvar('::snmp_trap2_sink')
  if $snmp_trap2_sink {
